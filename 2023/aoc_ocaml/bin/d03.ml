...
let matrix : char array array = Array.make_matrix ~dimx:rows ~dimy:cols Char.min_value
...
let part_1 =
  List.iteri lines ~f:(fun x line -> String.iteri line ~f:(fun y c -> matrix.(x).(y) <- c));
...
let part_2 =
  let nums_map = Hashtbl.create (Array.length matrix) in
  List.iteri lines ~f:(fun x line -> String.iteri line ~f:(fun y c -> matrix.(x).(y) <- c));
